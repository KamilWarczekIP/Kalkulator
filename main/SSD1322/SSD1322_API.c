...
{
	SSD1322_HW_INIT();
	SSD1322_HW_msDelay(100);                 //100ms delay
	SSD1322_HW_drive_RESET_low();  //Reset pin low
	SSD1322_HW_msDelay(100);                  //1ms delay
	SSD1322_HW_drive_RESET_high(); //Reset pin high
	SSD1322_HW_msDelay(500);                 //50ms delay
	...
	SSD1322_HW_msDelay(100);               //stabilize VDD
	SSD1322_API_command(0xAF);   //display on
	SSD1322_HW_msDelay(500);               //stabilize VDD
}
